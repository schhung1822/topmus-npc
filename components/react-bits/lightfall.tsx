...
export default function Lightfall({
  ...
  useEffect(() => {
    ...
    const removeResource = (resource: unknown, key: string) => {
      const method = resource && (resource as Record<string, unknown>)[key];
      if (typeof method === "function") (method as () => void).call(resource);
    };
    ...
    try {
      ...
      removeResource(programRef.current, "remove");
      removeResource(geometryRef.current, "remove");
      removeResource(meshRef.current, "remove");
      removeResource(renderer, "destroy");
    ...
    const render = (time: number) => {
      rafRef.current = null;
      if (paused || document.hidden || contextLost) return;
      ...
      if (mouseDampening > 0) {
        ...
        const factor = Math.min(1, 1 - Math.exp(-elapsed / Math.max(0.0001, mouseDampening)));
    ...
    const startRendering = () => {
      if (paused || document.hidden || contextLost || rafRef.current !== null) return;
    ...
    return () => {
      ...
      removeResource(geometryRef.current, "remove");
      removeResource(meshRef.current, "remove");
      removeResource(rendererRef.current, "destroy");
      programRef.current = null;
      geometryRef.current = null;
      meshRef.current = null;
      rendererRef.current = null;
